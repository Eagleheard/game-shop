...
const renderComponent = () =>
  ...
  const server = setupServer(
    rest.get('http://localhost:7000/api/game', (req, res, ctx) => {
...
describe('Home', () => {
  it('Should change to next or previous page after pressing arrow', async () => {
    ...
    await waitFor(() => expect(queryByTestId('preview-1')).toBeInTheDocument());
    expect(queryByTestId('container')).toBeInTheDocument();
    expect(queryByTestId('preview-0')).not.toBeInTheDocument();
  });
  /*act(async() => {
        ...
        currentPage += 1;
        expect(screen.findByTestId(`preview-${currentPage - 1}`)).not.toHaveClass(
          'preview__img--active',
        );
